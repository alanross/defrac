package com.adjazent.defrac.sandbox.experiments.samples.flutterman;

import com.adjazent.defrac.sandbox.experiments.samples.flutterman.visual.*;
import com.adjazent.defrac.sandbox.experiments.samples.flutterman.world.*;
...
public final class FluttermanGame
{
						   ...
						   @Nonnull final DisplayObjectContainer container )
	{
		...
		if( HEARTS_ENABLED )
		{
			final Layer heartLayer = container.addChild( new Layer( NUM_HEARTS ) );
...
//	@MacroJVM( "com.adjazent.defrac.sandbox.experiments.samples.flutterman.macro.Browser.openWebsite" )
//	@MacroWeb( "com.adjazent.defrac.sandbox.experiments.samples.flutterman.macro.Browser.openWebsite" )
//	@MacroA5D( "com.adjazent.defrac.sandbox.experiments.samples.flutterman.macro.Browser.openWebsite" )
	...
	private void handleGameOver()
	{
				...
				( frame == 20 || frame == 24 || frame == 32 || frame == 40 || frame == 48 || frame == 64 || frame == 100 ) )
		{
			...
			for( int i = 0; i < n; ++i )
			{
				...
				if( ++currentSparkle == NUM_SPARKLES )
				{
					currentSparkle = 0;
				}
			}
